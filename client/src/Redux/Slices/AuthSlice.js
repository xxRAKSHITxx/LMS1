...
const authSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(login.fulfilled, (state, action) => {
            console.log("Login Payload:", action?.payload); // Log role information

            localStorage.setItem("data", JSON.stringify(action?.payload?.user));
            localStorage.setItem("role", action?.payload?.user?.role || "USER"); // Default to "USER" if no role is provided
            localStorage.setItem("isLoggedIn", true);

            state.data = action?.payload?.user;
            state.role = action?.payload?.user?.role || "USER"; // Default to "USER" if no role is provided
            state.isLoggedIn = true;
        });
